...
def main():
    ...
    if args.do_train:
        ...
        if 'wp' in args.file_suffix and not template:
            ...
            train_features = convert_to_event_script_wp_features_update(train_data,
                                                                        input_event_num=args.input_event_num,
                                                                        use_relation=use_relation, no_struct=no_struct,
                                                                        is_eval=True, use_story_input=use_story_input,
                                                                        is_rl_sentence=False, is_otrl_ot=False)
        else:
            train_features = convert_graph_to_features_update(train_data, input_event_num=args.input_event_num,
                                                              use_relation=use_relation, no_struct=no_struct,
                                                              is_eval=True,
                                                              use_story_input=use_story_input, is_sentence_rl=False,
                                                              is_otrl_ot=False)
        num_train_steps = int(
