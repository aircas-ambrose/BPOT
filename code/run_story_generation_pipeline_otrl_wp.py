...
def fine_grained_for_ot(args, loss_bak, output_ids, reward, output_ids_s_index, cross_ot_allocate_bak):
    ...
    for i in range(args.train_batch_size):
        ...
        for j in range(batch_prompt_num[i]):
            ...
            sentence_end = output_ids_s_index[i][j] + 1
    ...
    for i in range(args.train_batch_size):
        ...
        batch_reward_loss.append(torch.tensor(reward_loss, dtype=torch.float32).to(device))
    for i in range(len(batch_reward_loss)):
        batch_reward_loss[i] = cross_ot_allocate_bak[i].size(1) * torch.mm(cross_ot_allocate_bak[i], batch_reward_loss[i].unsqueeze(-1)).squeeze()
        batch_reward_loss[i] = batch_reward_loss[i] / batch_reward_loss[i].mean()
    for i in range(args.train_batch_size):
        prompt_position = batch_prompt_position[i]
        reward_loss = batch_reward_loss[i]
...
def find_argument_interval_for_event(token_result, event_index, sep_token=25606, own_full_stop=False):
...
def get_event_representation(batch_event_position, last_hidden_states, device, event_extraction_model=None):
    """convert the word representation to event representation"""
    batch_size = last_hidden_states.shape[0]
    hidden_dim = last_hidden_states.shape[-1]
    batch_event_representation = []
    for i in range(batch_size):
        event_position = batch_event_position[i]
        sample_batch_event_representation = torch.zeros(len(event_position), hidden_dim)
        for j in range(len(event_position)):
            event_sample_position = event_position[j]
            sample_batch_event_representation[j, :] = torch.mean(
                last_hidden_states[i, event_sample_position[0]: event_sample_position[1], :], dim=0).to(device)
        batch_event_representation.append(sample_batch_event_representation)
    return batch_event_representation

def cost_matrix_torch(x, y):
    norm_x = x.div(torch.norm(x, p=2, dim=1, keepdim=True) + 1e-12)
    norm_y = y.div(torch.norm(y, p=2, dim=1, keepdim=True) + 1e-12)
    cos_distance = torch.mm(norm_x, torch.transpose(norm_y, 0, 1))
    cos_distance = 1 - cos_distance
    return cos_distance


def transport_matrix_torch(cost_matrix, n, m, device, beta=0.1, iteration=5):
    sigma = torch.ones(int(m), 1).to(device) / float(m)
...
def compute_regularization_loss(transport_matrix):
    """compute the binary cross entropy loss for transport matrix"""
...
def gaussian_cost_matrix_torch(x, y, gamma):
    norm_x = x.div(torch.norm(x, p=2, dim=1, keepdim=True) + 1e-12)
    norm_y = y.div(torch.norm(y, p=2, dim=1, keepdim=True) + 1e-12)
    norm_x_square = torch.square(norm_x).sum(dim=1, keepdim=True)
    norm_y_square = torch.square(norm_y).sum(dim=1, keepdim=True).reshape(1, -1)
    norm_xy = torch.mm(norm_x, torch.transpose(norm_y, 0, 1))
...
def ipot_distance_loss(x, y, device, beta=0.1, gamma=1, iteration=5):
    """完成整个ipot loss的计算步骤"""
    batch_size = len(x)
    batch_transport_matrix = []
    batch_ot_loss = []
    for i in range(batch_size):
        n = x[i].shape[0]
        m = y[i].shape[0]
        cost_matrix = gaussian_cost_matrix_torch(x[i], y[i], gamma)
        cost_matrix = cost_matrix.float().to(device)
        transport_matrix = transport_matrix_torch(cost_matrix, n, m, device,
                                                  beta=beta, iteration=iteration)
        temp = torch.mm(torch.transpose(cost_matrix, 0, 1), transport_matrix)
        ot_loss = temp.trace()
        batch_transport_matrix.append(transport_matrix.clone().detach())
        batch_ot_loss.append(ot_loss)
    return batch_ot_loss, batch_transport_matrix, None


def convert_scalar_index_to_tuple(scalar_index):
    batch_tuple_index = []
    batch_size = len(scalar_index)
    for i in range(batch_size):
        tuple_index = []
        event_tuple_start = 1
...
def main():
    ...
    parser.add_argument('--alpha',
                        type=float,
                        default=0.1,
                        help='the coefficience for the storyline ot loss')
    parser.add_argument('--beta',
                        type=float,
                        default=0.1,
                        help='the coefficience for the story ot loss')
    ...
    otrl_wp_tensorlist, otrl_wp_tensordataset = None, None

    if args.do_train:
        if os.path.isfile(os.path.join(args.data_dir, 'process_data', 'otrl_wp_train_indexlist.json')):
            otrl_wp_tensorlist = json.load(open(os.path.join(args.data_dir, 'process_data',
                                                                'otrl_wp_train_indexlist.json'), 'r'))
        if os.path.isfile(os.path.join(args.data_dir, 'process_data', 'otrl_wp_train_tensordataset.pkl')):
            otrl_wp_tensordataset = pickle.load(open(os.path.join(args.data_dir, 'process_data',
                                                                'otrl_wp_train_tensordataset.pkl'), 'rb'))
            num_train_steps = int(
                len(otrl_wp_tensordataset) / args.train_batch_size / args.gradient_accumulation_steps * args.num_train_epochs)

        if otrl_wp_tensorlist != None and otrl_wp_tensordataset != None:
            train_data = otrl_wp_tensordataset
            all_output_ids_index = otrl_wp_tensorlist['all_output_ids_index']
            all_output_ids_s_index = otrl_wp_tensorlist['all_output_ids_s_index']
            if use_story_input:
                all_story_inputs = otrl_wp_tensorlist['all_story_inputs']
        else:
            ...
            if 'wp' in args.file_suffix and not template:
                ...
                train_features = convert_to_event_script_wp_features_update(train_data, input_event_num=args.input_event_num,
            ...
            logger.info("input_id_size: {}, input_mask_size: {},"
                        ...
                        "output_id_s_size: {}, output_mask_s_size: {}, ".format(
                all_input_ids.size(), all_input_mask.size(), all_key_indices.size(),
                all_output_ids.size(), all_output_mask.size(),
                all_output_ids_stories.size(), all_output_mask_stories.size()))

            train_data = TensorDataset(all_input_ids, all_input_mask, all_key_indices,
                                       all_output_ids, all_output_mask, all_output_ids_stories,
                                       all_output_mask_stories)
        ...
        if 'wp' in args.file_suffix and not template:
            ...
            eval_features = convert_to_event_script_wp_features(eval_data, input_event_num=0, is_eval=True,
                                                          use_relation=use_relation, no_struct=no_struct,
                                                          use_story_input=use_story_input)
        ...
        for epoch in trange(int(args.num_train_epochs), desc="Epoch"):
            ...
            for step, batch in \
                ...
                input_ids, input_masks, instance_indices, output_ids, output_masks, \
                    output_ids_s, output_masks_s = batch
                output_index = [all_output_ids_index[i.item()] for i in instance_indices]
                output_ids_s_index = [all_output_ids_s_index[i.item()] for i in instance_indices]
                if 't5' in args.model:
                    ...
                    cross_ot_loss, cross_ot_allocate_bak, _ = ipot_distance_loss(
                    ...
                    cross_ot_loss = torch.tensor(cross_ot_loss).mean()
